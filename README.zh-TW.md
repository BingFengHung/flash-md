...
- 🔍 **強大全文搜尋 (Ctrl + F)**：即時相符筆數統計、輸入框自動聚焦、支援 `Enter` / `Shift + Enter` 與 `F3` / `Shift + F3` 上下筆跳轉定位、全 Unicode 中英文字元安全高亮。
...
| **`←` / `→`** | **切換檔案**：快速預覽同目錄下的上一個 / 下一個檔案 (附帶 `[3/18]` 序號) |
| **`↑` / `↓`** | **捲動瀏覽**：上下捲動瀏覽當前文件內容 (支援長按連續平滑捲動) |
| **`PageUp` / `PageDown`** | **快速翻頁**：在長文件中快速向上 / 向下翻頁瀏覽 |
| **`Home` / `End`** | **快速置頂/置底**：直接跳至文件最頂端或最末端 |
| **`Esc`** | 快速隱藏預覽視窗 / 關閉搜尋欄 |
| **`Ctrl + F`** | 開啟內文搜尋欄並自動聚焦輸入框 |
| **`Enter` / `F3`** | **搜尋下一筆**：在搜尋時自動滾動跳轉至下一個相符項目 |
| **`Shift + Enter` / `Shift + F3`** | **搜尋上一筆**：在搜尋時自動滾動跳轉至上一個相符項目 |
