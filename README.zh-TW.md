...
- 🧠 **Markdown 互動心智圖模式 (Mindmap Mode)**：按 **`F6`** 或 **`Ctrl + M`** 一鍵將 Markdown 標題大綱轉化為**現代互動式向量心智圖**！支援平滑貝茲曲線、滑鼠拖曳平移、滾輪縮放、子節點一鍵收折/展開 (`[+]`/`[-]`)，點擊節點即可秒速跳轉回正文對應章節！
- 📽️ **全螢幕簡報投影模式 (Slides Mode)**：按 **`F5`** 或 **`P`** 瞬間將 Markdown 依照 `---` 分隔線切換為全螢幕大字級簡報投影，支援 `←` / `→` / `Space` 鍵盤翻頁、懸浮控制條與全螢幕投影，臨時會議展示神器！
...
| **`F5`** 或 **`P`** | **簡報投影模式**：開啟 / 退出全螢幕 Markdown 簡報投影模式 (支援左右鍵翻頁) |
...
| **`Esc`** | 退出全螢幕 / 退出就地編輯模式 / 關閉搜尋欄 / 快速隱藏預覽視窗 |
| **`F11`** | **全螢幕模式**：全域切換全螢幕 / 視窗預覽模式 |
