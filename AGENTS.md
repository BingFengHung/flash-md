...
### 19. 全面單元測試與功能回歸防護規範 (Automated Unit Testing & Regression Guard)
- **新功能覆蓋與歷史功能劣化問題**：
  - 隨著專案模組擴充，未受測試保護的核心邏輯（如 Markdown 大綱解析、投影片分頁、心智圖樹狀幾何、搜尋索引匹配、設定檔序向化與自動更新版本比較）容易在重構或新增功能時被意外破壞。
- **標準單元測試架構**：
  - 核心模組（`markdown.rs`、`views/mindmap.rs`、`config.rs`、`updater.rs`）必須內嵌 `#[cfg(test)] mod tests` 單元測試套件。
  - 涵蓋場景：
    1. **語法與大綱解析**：各級標題提取、Slug 唯一性去重流水號、投影片 `---` 分割與 YAML frontmatter 過濾。
    2. **文字與表格引擎**：中英文字數統計、CSV/TSV 帶引號欄位解析、JSON 美化/壓縮與壞損容錯。
    3. **心智圖演算法**：樹狀層級建立、清單項嵌入、行內語法過濾與節點收折高度幾何計算。
    4. **設定與版本控制**：偏好設定 JSON 雙向讀寫與損壞回退、語意化版本號比較。
  - **CI/CD 自動化防線**：在 `ci.yml` 與 `release.yml` 的建置流程中強制加入 `cargo test --verbose`，只要有任何一項歷史或現有測試失敗，即刻中斷建置並阻擋發布，徹底根絕功能回歸劣化！
